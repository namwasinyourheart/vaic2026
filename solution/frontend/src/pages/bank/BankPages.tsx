...
function useDocuments() {
  ...
  const reload = async () => {
    ...
    try { setDocuments(await documentService.list()) } catch (value) { setError(value instanceof Error ? value.message : "API văn bản không phản hồi.") } finally { setLoading(false) }
  }
  useEffect(() => { void reload() }, [])
  return { documents, loading, error, reload }
}
function DocumentsUnavailable({ title, loading, error, reload }: { title: string; loading: boolean; error: string; reload: () => void }) {
    ...
    <DataStateBoundary title="Tra cứu văn bản">
      <AppLayout pageTitle="Tra cứu văn bản">
        <PageHeader
          ...
          subtitle="Tìm kiếm trên kho văn bản theo quyền truy cập"
        />
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Nhập tên, số hiệu hoặc nội dung văn bản..."
                className="w-full border rounded pl-9 pr-3 py-2 text-sm"
              />
            </div>
            <Btn variant="outline" onClick={() => setFilters((v) => !v)}>
          ...
          {filters && (
            <div className="grid grid-cols-5 gap-3 mt-4 pt-4 border-t">
              <Select
                ...
                options={[
                  { value: "", label: "Tất cả" },
                  ...["Quyết định", "Quy chế", "Hướng dẫn"].map((value) => ({
                    value,
                    label: value,
                  })),
                ]}
              />
              <Input label="Đơn vị ban hành" placeholder="Tất cả đơn vị" />
              <Input label="Lĩnh vực" placeholder="Tất cả lĩnh vực" />
              <Input label="Ngày hiệu lực từ" type="date" />
              <Select
                label="Hiệu lực"
                value={status}
                onChange={setStatus}
        ...
        <ApiDataState loading={loading} error={error} onRetry={() => void reload()} empty={!loading && !error && rows.length === 0} emptyTitle="Không tìm thấy văn bản">
          <div className="space-y-3">{rows.map((doc) => (
              <Card key={doc.id} className="p-4 flex items-start gap-4">
                <div className="w-11 h-11 rounded bg-red-50 text-[#C8102E] grid place-items-center">
                  <List size={20} />
                </div>
                <div className="flex-1 min-w-0">
                  <button
                    onClick={() =>
                      navigate(`/bank-employee/documents/${doc.id}`)
                    }
                    className="font-semibold text-sm hover:text-[#C8102E] text-left"
                  >
                    {doc.name}
                  </button>
                  <div className="text-xs font-mono text-gray-400 mt-1">
                    {doc.code} · {doc.type} · {doc.issuingUnit}
                  </div>
                  <p className="text-xs text-gray-600 mt-2 line-clamp-2">
                    {doc.description} Nội dung phù hợp với truy vấn được hiển
                    thị tại đây để người dùng đánh giá nhanh.
                  </p>
                  <div className="flex gap-2 mt-2">
                    <Badge
                      variant={
                        doc.effectiveStatus === "EFFECTIVE"
                          ? "effective"
                          : "warning"
                      }
                      label={statusLabel[doc.effectiveStatus]}
                    />
                    <span className="text-[10px] text-gray-400">
                      Hiệu lực: {doc.effectiveAt}
                    </span>
                  </div>
                </div>
                <div className="flex gap-2">
...
export function ClauseSearchPage() {
  ...
  const clauses = documents
    .flatMap((doc) => doc.clauses.map((clause) => ({ doc, clause })))
    .filter(
      (item) =>
        !query ||
        `${item.clause.content} ${item.clause.path}`
          .toLowerCase()
          .includes(query.toLowerCase()),
    )
    ...
    <DataStateBoundary title="Tìm theo điều khoản">
      <AppLayout pageTitle="Tìm theo điều khoản">
        <PageHeader
          ...
          subtitle="Tra cứu đến cấp chương, điều, khoản và điểm"
        />
        <Card className="p-4 mb-4">
          <div className="grid grid-cols-6 gap-3">
            ...
            <Input label="Số điều" placeholder="Ví dụ: 12" />
            <Input label="Số khoản" placeholder="Ví dụ: 2" />
            <Input label="Điểm" placeholder="Ví dụ: a" />
            <Select
              label="Hiệu lực"
              options={[
                { value: "", label: "Tất cả" },
        ...
        <div className="space-y-3">
          {clauses.map(({ doc, clause }) => (
            <Card key={clause.id} className="p-4">
              <div className="flex justify-between">
                <div>
                  <div className="text-xs text-[#C8102E] font-mono">
                    {doc.code}
                  </div>
                  <h3 className="font-semibold text-sm mt-1">{doc.name}</h3>
              ...
              <p className="text-sm leading-6 text-gray-700 bg-gray-50 rounded p-3 mt-3">
                {clause.content}
              </p>
              <div className="flex gap-2 mt-3">
                <Btn
                  size="sm"
                  onClick={() => navigate(`/bank-employee/documents/${doc.id}`)}
                >
                  Chi tiết
                </Btn>
                <Btn
                  size="sm"
                  variant="outline"
                  onClick={() => navigate("/bank-employee/timeline")}
                >
                  Timeline
                </Btn>
                <Btn
                  size="sm"
                  variant="outline"
                  onClick={() => navigate("/bank-employee/compare")}
                >
                  So sánh
                </Btn>
...
export function DocumentDetailPageNew() {
  ...
  const load = async () => { setLoading(true); setError(""); try { setDoc(await documentService.get(id)) } catch (value) { setError(value instanceof Error ? value.message : "API chi tiết văn bản không phản hồi.") } finally { setLoading(false) } }
            ...
            <div className="flex h-[520px]">
              <aside className="w-60 border-r p-3 overflow-y-auto">
                ...
                {[
                  ...
                  "Chương II. Nội dung",
                  "Điều 5. Lãi suất",
                  "Khoản 2. Kỳ hạn 12 tháng",
                ].map((text) => (
                  <button
                    key={text}
                    className="block text-left w-full text-xs p-2 hover:bg-gray-50"
                  >
                    {text}
                  </button>
                ))}
              ...
              <article className="flex-1 p-5 overflow-y-auto">
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    className="border rounded pl-8 pr-3 py-1.5 text-xs w-72"
                  />
                </div>
                ...
                <p className="mt-5 text-sm leading-7">
                  Căn cứ các quy định pháp luật hiện hành và quy chế hoạt động
                  của Ngân hàng TMCP Sài Gòn – Hà Nội;
                </p>
                <h3 className="font-bold mt-5">Điều 5. Quy định áp dụng</h3>
                <p
                  className={`mt-2 text-sm leading-7 ${
                    contentQuery ? "bg-yellow-100" : ""
                  }`}
                >
                  {doc.clauses[0]?.content}
                </p>
              </article>
            </div>
          )}
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-400">
                      ...
                      {version.effectiveTo || "Hiện tại"} ·{" "}
                      {version.changeDocument}
                    </div>
                  </div>
                  <Badge
                    variant={
                  ...
                  <input
                    type="checkbox"
                    className="ml-3"
                    aria-label={`Chọn ${version.label} để so sánh`}
                  />
                </div>
              ))}
            </div>
          ...
          <div key={event.id} className="flex gap-4 pb-5">
            ...
            <div className="border rounded p-4 flex-1">
              ...
              <div className="text-xs text-gray-500 mt-1">
                Văn bản tạo thay đổi: {event.document}
              </div>
              <div className="grid grid-cols-2 gap-3 mt-3">
                <div className="bg-red-50 text-red-700 p-3 rounded text-xs">
                  <b>Trước:</b> {event.before}
                </div>
                <div className="bg-green-50 text-green-700 p-3 rounded text-xs">
                  <b>Sau:</b> {event.after}
                </div>
              </div>
            </div>
        ...
        .map((relation) => (
          ...
          >
            ...
            <div className="flex-1 ml-3">
              ...
              <div className="text-xs text-gray-400">
                {relation.type} · Áp dụng từ {relation.startDate}
              </div>
            </div>
...
export function ComparePage() {
  ...
  if (loading || error) return <DocumentsUnavailable title="So sánh phiên bản" loading={loading} error={error} reload={reload} />
  if (!doc) return <AppLayout pageTitle="So sánh phiên bản"><ApiDataState loading={false} empty><div /></ApiDataState></AppLayout>
  const pair = swapped
        ...
        <PageHeader
          ...
          actions={
            <>
              ...
              <Btn variant="outline" onClick={() => setSwapped((v) => !v)}>
                <GitCompareArrows size={14} /> Đổi vị trí
              </Btn>
            </>
          }
        />
        <div className="grid grid-cols-2 gap-4">
          {pair.map((version, index) => (
            <Card key={`${version.id}-${index}`}>
              <div className="p-4 border-b">
                ...
                <div className="text-xs text-gray-400">
                  Hiệu lực từ {version.effectiveFrom}
                </div>
              </div>
              <div className="p-5 text-sm leading-7">
                Lãi suất kỳ hạn 12 tháng là{" "}
                <mark
                  className={index ? "bg-green-200" : "bg-red-200 line-through"}
                >
                  {version.content.match(/[0-9],[0-9]%/)?.[0]}
                </mark>{" "}
                tại quầy.{" "}
                {onlyDiff
                  ? ""
                  : "Các nội dung khác của văn bản được giữ nguyên."}
              </div>
            </Card>
          ))}
        </div>
    ...
    <DataStateBoundary title="Timeline sửa đổi">
      <AppLayout pageTitle="Timeline sửa đổi">
        <PageHeader
          ...
          subtitle="Theo dõi thay đổi của văn bản và điều khoản"
        />
        <Card>
    ...
    <DataStateBoundary title="Văn bản liên quan">
      <AppLayout pageTitle="Văn bản liên quan">
        <PageHeader
          ...
          subtitle="Quan hệ tham chiếu, sửa đổi và thay thế"
          actions={
            <div className="flex border rounded">
              ...
              <button
                ...
                className={`p-2 ${view === "graph" ? "bg-gray-100" : ""}`}
              >
                <Network size={15} />
              </button>
            </div>
        ...
        <Card>
          {view === "list" ? (
            <RelationList documents={documents.slice(0, 6)} />
          ) : (
            <div className="h-[520px] grid place-items-center bg-[#F8FAFC]">
              <div className="relative w-[600px] h-[360px]">
                {documents.slice(0, 6).map((doc, index) => (
                  <div
                    key={doc.id}
                    className={`absolute rounded-full grid place-items-center text-white text-xs text-center p-3 ${
                      index === 0
                        ? "w-28 h-28 bg-[#C8102E] left-60 top-28"
                        : "w-24 h-24 bg-[#192B4B]"
                    }`}
                    style={
                      index
