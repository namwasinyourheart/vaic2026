...
type BadgeVariant = "queued" | "processing" | "pending" | "completed" | "failed" | "reprocessing" | "draft" | "approved" | "rejected" | "effective" | "partially_effective" | "superseded" | "expired" | "future_effective" | "detected" | "confirmed" | "resolved" | "active" | "locked" | "info" | "warning"
...
export function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium font-mono ${BADGE_STYLES[variant]}`}
    >
...
export function Card({
  ...
  return (
    <div
      className={`bg-white rounded-[6px] border border-[#DDE1E9] ${className}`}
    >
      {children}
    </div>
  )
...
export function Btn({
  ...
  const base =
    "inline-flex items-center gap-1.5 font-medium rounded transition-all cursor-pointer disabled:opacity-50"
  const variants: Record<string, string> = {
    primary:
      "bg-[#C8102E] text-white hover:bg-[#a50d25] border border-[#C8102E]",
    secondary:
      "bg-[#1A2B4A] text-white hover:bg-[#142038] border border-[#1A2B4A]",
    ghost: "text-gray-600 hover:bg-gray-100 border border-transparent",
    danger: "bg-red-600 text-white hover:bg-red-700 border border-red-600",
    outline:
      "bg-white text-gray-700 border border-[#DDE1E9] hover:border-gray-400 hover:bg-gray-50",
  }
    ...
    <div className="flex border-b border-[#DDE1E9] gap-0">
      {tabs.map((tab) => (
        <button
          ...
          className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors -mb-px cursor-pointer ${active === tab.key
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div
        className={`bg-white rounded-lg shadow-xl w-full ${widths[size]} border border-[#DDE1E9]`}
      >
...
export function ApiDataState({
  loading,
  error,
  empty,
  onRetry,
  emptyTitle = "Chưa có dữ liệu",
  emptyDescription = "API đã phản hồi thành công nhưng chưa có dữ liệu để hiển thị.",
  children,
}: {
  loading: boolean
  error?: string
  ...
  if (loading) {
    return (
      <Card className="min-h-44 grid place-items-center">
        <div className="text-center px-6 py-10">
          ...
          <div className="hidden text-xs text-gray-500 mt-1">
            Dữ liệu chưa được xác nhận. Vui lòng không đóng trang.
          </div>
          <div className="hidden flex justify-center gap-2 mt-4">
            {[1, 2, 3].map((item) => (
              <span
                key={item}
                className="block h-2 w-16 rounded bg-gray-200 animate-pulse"
              />
            ))}
          </div>
        </div>
  ...
  if (error) {
    return (
      <Card className="min-h-44 grid place-items-center border-red-200 bg-red-50/40">
        <div role="alert" className="text-center max-w-xl px-6 py-10">
          <AlertTriangle className="text-red-600 mx-auto" size={32} />
          <div className="font-semibold text-red-800 mt-3">
            Không thể tải dữ liệu từ API
          </div>
          <div className="text-xs text-red-700 mt-1">
            Dữ liệu trên màn hình chưa được tải và không được xem là danh sách
            rỗng.
          </div>
          <div className="text-xs text-gray-600 bg-white border border-red-100 rounded p-2 mt-3 break-words">
            {error}
          </div>
          {onRetry && (
            <Btn variant="outline" className="mt-4" onClick={onRetry}>
              <RefreshCw size={13} /> Thử tải lại
            </Btn>
          )}
        </div>
      </Card>
...
export function Avatar({
  ...
  return (
    <div
      className={`${sz} rounded-full bg-[#1A2B4A] text-white flex items-center justify-center font-semibold flex-shrink-0`}
    >
      {initials}
    </div>
  )
...
export function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/25">
      ...
      />
      <aside
        className={`absolute right-0 top-0 bottom-0 ${width} max-w-full bg-white shadow-2xl flex flex-col`}
      >
...
export function Toast({
  ...
  return (
    <div
      role="status"
      className={`fixed bottom-5 right-5 z-[60] rounded-lg px-4 py-3 text-xs text-white shadow-xl ${tone === "error"
          ? "bg-red-700"
          : tone === "info"
            ? "bg-blue-700"
            : "bg-gray-900"
    ...
    >
      {message}
      {onClose && (
        <button onClick={onClose} className="ml-3 opacity-70">
          ×
        </button>
      )}
    </div>
